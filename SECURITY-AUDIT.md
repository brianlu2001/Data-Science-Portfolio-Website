...
3. **Prepare the deployed database.** Review/run `migrations/security.sql` with a migration role; the runtime account needs data access and sequence usage, not DDL privileges. Existing installations may already have the required tables/columns. This script was not run against production.
4. **Verify deployed environment variables.** SESSION_SECRET must contain at least 32 random characters; ADMIN_PASSWORD must be a unique password of at least 16 characters. Canonical luki90.com origins and Vercel-provided deployment origins are allowed; list any additional trusted custom origins in APP_ORIGINS. Missing/weak credentials fail closed.
5. **Complete production operational checks.** Verify HTTPS/Secure cookies, platform MFA/WAF settings, restricted DB/blob privileges, backups/restores, dependency alerts and monitoring. Run/schedule the supplied retention script after reviewing its deletion/minimization policy.
